...
public class SpellDB {
		...
		if (held.getClassS()==Classes.GOTT && gegner.getH()<10) {
			...
			// --- ZOMBIE ---
				...
				if (held.getH()<=0) {
					...
					return normal(held, gegner);
				} else return hirnwurf(held, gegner);
	...
	public static double g(Hero h, Hero g) {
		if (g.getClassS() == Classes.SHELDON ) {
			...
			System.out.println(special+"* JESUS: Du kannst mich aufhängen, mich quälen, aber ich werde NIIIIIIIEMALS STERBEN!!! "+special);
			return 0;
		} else if (g.getClassS()==Classes.GOTT) {
	...
	public static double m(Hero h, Hero g) {
		if (g.getClassS() == Classes.SHELDON ) {
			...
			System.out.println("~ Dein Held führt einen räudigen Zaubertrick aus ~");
			System.out.println("Magiekraft sinkt auf "+h.getM()*0.5);
			h.setM(h.getM()*0.5);
			return Math.ceil(h.getM()*0.5);
			} else if (chance(40)) {
				System.out.println("///> Gedankenraub <///");
				System.out.println("~ Du bündelst Hirn und Magiekraft um einen Energiestrahl zu erzeugen! ~");
	...
	public static double Ansturm(Hero h, Hero g) {
		if (g.getClassS() == Classes.DRACHE || g.getClassS() == Classes.DRACHE) {
			System.out.println("* Gegner fliegt. Worauf willst du stürmen?! *");
			h.reG(1.5);
			System.out.println("*"+h.getpName()+" ist näher am Gegner! Geschick steigt auf "+h.getG()+"*");
			return 0;
		} else {
	...
	public static double Fury(Hero h, Hero g) {
		if (g.getClassS() == Classes.DRACHE) {
			...
			superp("Beim 2. Angriff schlägt dir der Gnom in die Nüsse. "+g.getpName()+"lacht dich aus.");
			h.reG(0.5);
			...
			System.out.println("$ Beim 2. Angriff rutschst du aus und fällst um!"+g.getpName()+"lacht dich aus. $");
			h.reG(0.8);
			return normal(h,g)*1.2;
		}
	}
	...
	public static double Zombieschrei(Hero h, Hero g) {
		if (g.getClassS() == Classes.SHELDON ) {
			...
			if (r>0) {
				r++;
				System.out.println("Zombieschrei war so laut, dass "+r+" Zombies nun mit dir kämpfen!");
	...
	public static double hirnwurf(Hero h, Hero g) {
		...
		if (g.getClassS() == Classes.SHELDON ) {
			System.out.println("* Zombieschlag tötet Sheldon! *");
			return g.getL();
		} else {
	...
	if (g.getClassS() == Classes.SHELDON ) {
		...
		System.out.println("* LAZ0000R BEEAAM !!! trifft die Flügel des Drachen. Er fällt zu Boden! *");
		g.ausre("a", 0.2);
		g.ausre("a", 0.2);
	...
	public static boolean chance(double prozent) {
		if (prozent>100 || Math.ceil(Math.random()*(100/prozent)) == 1 ) return true;
		else return false;
	}
