...
public class SpellDB {
    ...
    public static double g(Hero h, Hero g) {
        if (g.getClassS() == Classes.SHELDON) {
            ...
            System.out.println(special + "* JESUS: Du kannst mich aufhï¿½ngen, mich quï¿½len, aber ich werde NIIIIIIIEMALS STERBEN!!! " + special);
            return 0;
        } else if (g.getClassS() == Classes.GOTT) {
    ...
    public static double m(Hero h, Hero g) {
        if (g.getClassS() == Classes.SHELDON) {
            ...
            System.out.println("* Sheldon: Magie gehorscht keinen Physikalischen Gesetzen!!! WIE KANN ES MIR SCHADEN MACHEN!ï¿½)3t4? *");
            h.setM(0);
            return h.getM();
        } else if (g.getClassS() == Classes.JESUS) {
            ...
            if (chance(30)) {
                ...
                System.out.println("~ Dein Held fï¿½hrt einen rï¿½udigen Zaubertrick aus ~");
                System.out.println("Magiekraft sinkt auf " + h.getM() * 0.5);
                h.setM(h.getM() * 0.5);
                return Math.ceil(h.getM() * 0.5);
            } else if (chance(40)) {
                System.out.println("///> Gedankenraub <///");
                System.out.println("~ Du bï¿½ndelst Hirn und Magiekraft um einen Energiestrahl zu erzeugen! ~");
    ...
    public static double Ansturm(Hero h, Hero g) {
        if (g.getClassS() == Classes.DRACHE || g.getClassS() == Classes.DRACHE) {
            System.out.println("* Gegner fliegt. Worauf willst du stï¿½rmen?! *");
            h.reG(1.5);
            System.out.println("*" + h.getpName() + " ist nï¿½her am Gegner! Geschick steigt auf " + h.getG() + "*");
            return 0;
        } else {
    ...
    public static double Fury(Hero h, Hero g) {
        if (g.getClassS() == Classes.DRACHE) {
            ...
            superp("Beim 2. Angriff schlï¿½gt dir der Gnom in die Nï¿½sse. " + g.getpName() + "lacht dich aus.");
            h.reG(0.5);
            ...
            System.out.println("$ Beim 2. Angriff rutschst du aus und fï¿½llst um!" + g.getpName() + "lacht dich aus. $");
            h.reG(0.8);
            return normal(h, g) * 1.2;
        }
    }
    ...
    public static double Zombieschrei(Hero h, Hero g) {
        ...
        if (g.getClassS() == Classes.ZOMBIE) {
            ...
            if (r > 0) {
                r++;
                System.out.println("Zombieschrei war so laut, dass " + r + " Zombies nun mit dir kï¿½mpfen!");
    ...
    public static double zombieschlag(Hero h, Hero g) {
        if (g.getClassS() == Classes.SHELDON) {
            System.out.println("* Zombieschlag tï¿½tet Sheldon! *");
            return g.getL();
        } else {
